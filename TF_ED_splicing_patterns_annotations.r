...
all_uniprot <- unique(alluni) ## 1,469 TFs have a 1-1 mapping with an Ensembl transcript
...
##overlap of ED ensembl protein ids and the ensembl ids of the canonical uniprot seq
ovrlp <- intersect(ed_file2$`Uniprot ID`, tf_ensemb_map$Uniprotswissprot)
ed_file <- ed_file[ed_file$`Uniprot ID` %in% ovrlp, ] ## 847 EDs out of the 924 EDs remain

tuniprot <- intersect(all_uniprot, ed_file$`Uniprot ID`)
## Of the 1,469 TFs with a matching Ensembl transcript, 538 have ED information ----

fed_file <- ed_file[ed_file$`Uniprot ID` %in% tuniprot, ]
