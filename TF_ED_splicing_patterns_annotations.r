...
ovrlp <- intersect(ed_file2$`ENSEMBL protein ID`, tf_ensemb_map$Ensembl_protein_id)
ed_file <- ed_file[ed_file$`ENSEMBL protein ID` %in% ovrlp, ] ## 730 EDs out of the 924 EDs remain

tuniprot <- intersect(all_uniprot, ed_file$`Uniprot ID`)
## Of the 1,469 TFs with a matching Ensembl transcript, 536 have ED information ----
