...
##--- which of the master splicing events affect DBDs ----------------------------------
...
for(k in 1:length(all_cancer)){
    ...
    if(length(tgenes) != 0){
        for(j in 1:length(tgenes)){
            tempy <- tempx[tempx$symbol == tgenes[j],]
            teves <- tempy$as_id
            tst <- tempy$splice_type
            tuni <- tf_ensemb_map[tf_ensemb_map$HGNC_symbol == tempy$symbol[1], ]$Uniprotswissprot[1]
            if(is.na(tuni)){
                counter <- counter+1
                next
            ...
            for(i in 1:length(teves)){
                ...
                mdd <- c(mdd, tempy$MEDIAN_DIFF[i])
                as <- c(as, tempy$as_id[i])
                tcancer <- c(tcancer, all_cancer[k])
                gene <- c(gene, tempy$symbol[i])
            }
        }
...
tdata <- data.frame(CANCER=tcancer, GENE=gene, ASID=as,DBD=odbd, MD=mdd)

###--------------------------------------------------------------------------------------

###--- which of the master splicing events have directional GRNs information -----------
library(dorothea)
all_map <- data.table::fread('../data/ensembl_name_map.txt')
all_mapx <- unique(all_map[,c(1,8,9)])
...
for(k in 1:length(all_cancer)){
    tgenes <- top5xpr[[k]]
    temp_grn <- conf_grn_symbol[conf_grn_symbol$TF_symbol %in% tgenes, ] 
}
