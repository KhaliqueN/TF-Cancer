...
strip.text = element_text(size = basesize * 0.8), axis.title=element_text(basesize * 0.8))+
guides(fill=guide_legend(title="Percent Spliced In",ncol=1))#guides(fill='none')
ggsave(p,filename=paste0(save_dir,"/Events_perturbing_overlap.png"),width=7, height=3, dpi=400)
...
events_to_consider <- union(num_events_combs_pos[[10]], num_events_combs_neg[[10]])
temp <- data.table::fread(all_files[idd], sep='\t')
tempg <- temp[temp$as_id %in% events_to_consider, ]$symbol
# temp_gene <- conf_grn[conf_grn$`tf` %in% tempg, ]$`target`
...
strip.text = element_text(size = basesize * 0.8), axis.title=element_text(basesize * 0.8))+
guides(fill=guide_legend(title="Percent Spliced In",ncol=1))#guides(fill='none')
ggsave(p,filename=paste0(save_dir,"/Unique_sig_events_TFs.png"),width=5.5, height=3, dpi=400)
...
for(k in 1:length(all_cancer)){
    ...
    events_tf_surv[[k]] <- intersect(tempy$SE, events_tf)
    events_tf_surv_c <- c(events_tf_surv_c, length(events_tf_surv[[k]]))
    events_tf_surv_p <- c(events_tf_surv_p, (length(events_tf_surv[[k]])/length(events_tf)*100))
