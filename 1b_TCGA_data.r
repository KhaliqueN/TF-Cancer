##############################################################################################
# Purpose: Save the filtered PPSI value files --
##############################################################################################
...
for(k in 1:length(manifests)){
	...
	##-- only keep the paired samples ----
	col_pos <- which(colnames(temp_psi) %like% 'TCGA')
	keep_cols <- c(seq(1,10),col_pos)
	temp_psi1 <- temp_psi[ ,keep_cols]

	temp_psi2 <- temp_psi[14:length(temp_psi[[1]]), col_pos]

    ##-- Filter to only retain samples from TCGA gene expression data
	...
	normal_list_filt <- lapply(normal_list, function(x) x[x != 'null'])
	cancer_list <- as.list(as.data.frame(t(cancer[,order(colnames(cancer))])))
	cancer_list_filt <- lapply(cancer_list, function(x) x[x != 'null'])
	...
	diff <- mapply(function(x,y) median(as.numeric(y)) - median(as.numeric(x)), normal_list_filt, cancer_list_filt)
