...
tempfun <- function(tempo_data, tempxz, xxp){
    ...
    temp1_c <- plyr::count(tempo_data[,c(14,as.numeric(xxp))])
    colnames(temp1_c) <- c('DBD','AS','freq')

    xdbd <- c()
    for(ii in 1:length(temp1_c[[1]])){
        xdbd <- c(xdbd, length(which(tempxz$DBD == temp1_c$DBD[ii])))
    } 
    temp1_c$len <- xdbd
    ...
    if(length(wh) != 0){
        ...
        for(i in 1:length(temp1_c1[[1]])){
            ...
            for(j in 1:length(tempv)){
                temp1_c2 <- rbind(temp1_c2, data.frame(DBD=temp1_c1$DBD[i], AS=tempv[j], frac=temp1_c1$frac[i]))
            }
        }
    }else{
    ...
    temp1_cc2 <- aggregate(temp1_c2$frac, by=list(DBD=temp1_c2$DBD, AS=temp1_c2$AS), FUN=sum)
    colnames(temp1_cc2) <- c('DBD','AS','frac')
    return(temp1_cc2)
...
tdata <- data.frame(CANCER=tcancer, SYMBOL=tgenet, AS=asid, SPLICE_TYPE=tsplice, DBD=odbd, MEAN_CANCER=corr1, 
    MEAN_NORMAL=corr2, MEAN_DIFF=corr3, FDR=corr4)
...
strip.text = element_text(size = basesize), 
...
ggsave(ppx,filename=paste0(save_dir, "/Events_types_perturbing_DBDs.png"),width=3, height=3.2, dpi=600)
...
tdatat <- data.frame(tt=c('Supplementary Table S5: List of DBD-perturbing PTSEs',
...
tolabel <- subset(pdata, ASID == '14951' & abs(MEAN_DIFF) > 0.4 & AA == 100)$ID
...
p <- p + 
scale_x_continuous(name="% of DBD residues", limits=c(0,110), breaks = seq(0, 100, by = 20)) + 
...
for(k in 1:length(pdatamul[[1]])){
    ...
    for(j in 1:length(tempv)){
        pdatauni <- rbind(pdatauni, data.frame(SYMBOL=pdatamul$SYMBOL[k], DBD=tempv[j]))
    }
}
...
p <- p + theme_classic(base_size = basesize) + labs(x = "Sample", y = "Gene") +
  ...
  guides(fill=guide_legend(title="# of PTSEs", ncol=5))+
  theme(axis.text.x = element_text(size = basesize * 1,angle = 60, hjust = 1,vjust=1, colour = "black"), 
                         axis.text.y = element_text(size = basesize * 1, colour='black'), 
                         panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.title.x = element_text(size=basesize*1),axis.title.y = element_text(size=basesize*1), 
        ...
        legend.text=element_text(size=basesize), legend.position="bottom", legend.key.height= unit(0.25, 'cm'),
    legend.key.width= unit(0.25, 'cm'))
ggsave(p,filename=paste0(save_dir,'/Perturbed_DBDs_tile.png'),width=3.5, height=4.5, dpi=500)
