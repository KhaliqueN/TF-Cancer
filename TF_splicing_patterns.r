...
tdatat <- data.frame(tt=c('Supplementary Table S2: List of cancer type-related PTSEs',
    'Explanation of the table fields are outlined below', 'SYMBOL: HGNC gene symbol',
...
theme_classic()+
theme(axis.text.x = element_text(size = 1*basesize, angle = 60, vjust=1, hjust=1, colour = "black"),
...
panel.grid.major = element_blank(),
panel.grid.minor = element_blank(),
strip.text = element_text(size = basesize*1), 
axis.title=element_text(size=basesize*1), 
legend.position="right", 
legend.text=element_text(size=basesize*1), 
legend.title=element_text(size=basesize*1))+
...
geom_text(data=pmdn, aes(y=pos, x=CANCER,label=count), position=position_dodge(width=0.9),hjust=0.5, vjust=0.5, angle=75, size=2.8)+
...
##--- column name explanations --
tdatat <- data.frame(tt=c('Supplementary Table S3: List of PTSEs and their cancer type occurrences',
...
# geom_text(aes(label=count), position=position_dodge(width=0.9),hjust=0, vjust=0, angle=75, size=3)+
geom_text(aes(x=cancer, y=vax, label=value), position=position_stack(vjust=1), size=3.2)+
theme(axis.text.x = element_text(size = basesize, angle = 60, hjust = 0.5,vjust=0.5, colour = "black"),
axis.text.y = element_text(size = basesize, angle = 0, hjust = 0.5,vjust=0.5, colour = "black"),
panel.grid.major = element_blank(),
panel.grid.minor = element_blank(),  
...
##--- column name explanations --
tdatat <- data.frame(tt=c('Supplementary Table S4: List of PTSEs uniquely occuring in a cancer type',
    'Explanation of the table fields are outlined below',
   'AS_ID: Alternative splicing ID provided by TCGA SpliceSeq',
...
pdata$lhr <- log2(pdata$Hazard_Ratio)

# pdata1 <- pdata[pdata$Hazard_Ratio < 3, ] ## filter outliers
pdata1 <- pdata[pdata$lhr < 10, ] ## filter outliers


p <- ggplot(pdata1, aes(lhr, MEAN_DIFF, color=CANCER, label=PL)) + 
# p <- ggplot(pdata, aes(AA, MEAN_DIFF, color=CANCER)) + 
geom_point()+
theme(legend.text=element_text(size=12))
basesize <- 8
p <- p + 
scale_x_continuous(name="log2(hazard ratio)") + 
...
# guides(color='none')
guides(color=guide_legend(title="Cancer type", ncol=2, override.aes = list(size = 2)))
ggsave(p,filename=paste0(save_dir,"/Unique_HR.png"),width=6, height=2.8, dpi=500)
