...
for(k in 1:length(all_cancer)){
    ...
    wh <- which(tempx$CI_Type == 'disease\nfree\ninterval')
    if(length(wh) > 0){tempx$CI_Type[wh] <- 'DFI'}

    wh <- which(tempx$CI_Type == 'progression\nfree\ninterval')
    if(length(wh) > 0){tempx$CI_Type[wh] <- 'PFI'}

    wh <- which(tempx$CI_Type == 'overall\nsurvival')
    if(length(wh) > 0){tempx$CI_Type[wh] <- 'OS'}

    wh <- which(tempx$CI_Type == 'disease\nspecific\nsurvival')
    if(length(wh) > 0){tempx$CI_Type[wh] <- 'DSS'}
    ...
    scale_fill_gradientn(colors=c('#a50026','#d73027','#f46d43','#fdae61','#fee090','#e0f3f8','#abd9e9','#74add1','#4575b4','#313695'),
        limits=c(-3, 3), breaks=seq(-3, 3, 0.6))+
    theme_bw()+theme(axis.text.x = element_text(size = 0.6*basesize, angle = 60, vjust=0.5, hjust=0.5, colour = "black"),
        ...
        panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "black"), axis.title=element_text(size=basesize * 0.8),
    ...
    ggsave(ppx,filename=paste0(save_dir, "/",all_cancer[k],".png"),width=7, height=5, dpi=700)
