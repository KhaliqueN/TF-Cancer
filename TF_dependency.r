...
guides(color='none')
ggsave(ppx,filename=paste0(save_dir, "/Scatter_dependency.png"),width=2, height=3, dpi=500)
...
ggsave(combined_plot,filename=paste0(save_dir, "/Dependency.png"),width=18, height=16, dpi=1200)
...
tdatat <- data.frame(tt=c('Supplementary Table S8: Cancer dependency information of PTSE-related TFs',
    'Explanation of the table fields are outlined below','CANCER: TCGA cancer code',
...
for(k in 1:length(filt_cancer)){
    ...
    temp <- pdatal[pdatal$CANCER == filt_cancer[k], ]
    if(nrow(temp) == 0){next}
    asid <- c()
    astype <- c()
    tempcol <- temp$GENE
