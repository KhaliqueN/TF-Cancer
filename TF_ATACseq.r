...
tdatat <- data.frame(tt=c('Supplementary Table S7: Correlation of PSI values of PTSEs with footprinting data',
...
openxlsx::addWorksheet(wb1, sheetName = 'Table S7')
openxlsx::writeData(wb1, sheet = 'Table S7', TF_FP_allx)
openxlsx::saveWorkbook(wb1, paste0(save_dir,'/Supplementary_Table_S7.xlsx'), overwrite = T)

# data.table::fwrite(TF_FP_allx, paste0(save_dir,'/Supplementary_Table_S7.csv'),sep='\t',row.names=FALSE, quote=FALSE)
...
# tolabel <- subset(tempd3, IDD %in% tomark & CANCER=='LUSC' & ASID=='63413')$ID
tolabel <- subset(tempd3, IDD %in% tomark & ASID=='63413')$ID
...
ggsave(ppx,filename=paste0(save_dir, "/Footprint_depth_flank_sig.png"),width=4.5, height=3, dpi=500)


###------------------------------------------------------------------------
##--- extract number from the saved supplementary table ---
tempg <- openxlsx::read.xlsx(paste0(save_dir,'/Supplementary_Table_S7.xlsx'),2)
tempg1 <- tempg[tempg$FDR < 0.05, ]
print(unique(paste0(tempg1$CANCER,'_',tempg1$AS_ID)))
print(unique(tempg1$CANCER))
print(unique(tempg1$AS_ID))
print(unique(tempg1$SYMBOL))
