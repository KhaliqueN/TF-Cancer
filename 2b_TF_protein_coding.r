##############################################################################################
# Purpose: For each cancer type, see which events affect protein coding regions 
##############################################################################################
...
save_dir <- '../results_new/TF_coding'
...
##--- Save all coding perturbing events -----------------------------


wb1 <- openxlsx::createWorkbook(paste0(save_dir,'/Events_perturbing_coding_region.xlsx'))
...
for(k in 1:length(all_cancer)){
    ...
    if(length(ect) != 0){
        ...
        for(j in 1:length(tempyy[[1]])){
            ...
            if(tempyy$splice_type[j] == 'AP'){
                ...
                start <- c(start, min(y1$UNIPROT_SEQ_NUM))
                end <- c(end, max(y1$UNIPROT_SEQ_NUM))
                seq <- c(seq, paste(y1$UNIPROT, collapse=''))
    ...
    ## save excel sheet ----
    tdatat <- data.frame(GENE=tgene, AS=asidt, START=start, END=end, SEQUENCE=seq, MEDIAN_CANCER=corr1t, MEDIAN_NORMAL=corr2t, MEDIAN_DIFF=corr3t, FDR=corr4t)
    ...
    openxlsx::saveWorkbook(wb1, paste0(save_dir,'/Events_perturbing_coding_region.xlsx'), overwrite = T)
}
...
ggsave(p,filename=paste0(save_dir,"/Events_perturbing_coding.png"),width=3.5, height=3, dpi=400)
...
ggsave(ppx,filename=paste0(save_dir, "/Events_types_perturbing_coding.png"),width=7, height=3, dpi=400)
...
ggsave(p,filename=paste0(save_dir,"/Perturbed_events_patients_coding.png"),width=4.5, height=3, dpi=600)
...
strip.text = element_text(size = basesize * 0.8), axis.title=element_text(basesize * 0.8))+
guides(color='none')
ggsave(p,filename=paste0(save_dir,"/Sig_events_TFs_coding_dist.png"),width=5, height=3, dpi=400)
...
strip.text = element_text(size = basesize * 0.8), axis.title=element_text(basesize * 0.8))+
guides(fill=guide_legend(title="Percent Spliced In",ncol=1))#guides(fill='none')
ggsave(p,filename=paste0(save_dir,"/Events_perturbing_coding_overlap.png"),width=3.5, height=3, dpi=400)
...
strip.text = element_text(size = basesize * 0.8), axis.title=element_text(basesize * 0.8))+
guides(fill='none')
ggsave(p,filename=paste0(save_dir,"/TFs_with_affected_coding.png"),width=3.5, height=3, dpi=400)
...
color <- rev(c('#a50026','#d73027','#f46d43','#fdae61','#fee090','#e0f3f8','#abd9e9','#74add1','#4575b4','#313695'))
breaks <- seq(-1, 1, 0.2)
p <- pheatmap(pdx,fontsize=3, color=color, breaks=breaks, cluster_rows=FALSE, cluster_cols=FALSE,cellheight=5, cellwidth = 5)
ggsave(p,filename=paste0(save_dir, "/Pancancer_events_coding.png"),width=7, height=5, dpi=600)
