...
##--- Values for ploting the protein positions ----
tempf <- openxlsx::read.xlsx('results_rep/Lambourne/Supplementary_Table_S9.xlsx', 2)
tempf_sym <- tempf$SYMBOL

for(k in 1:length(tempf_sym)){

    tempf_uni <- temp_map[temp_map$HGNC %in% tempf_sym[k], ]$Uniprotswissprot
    temp_as <- tempf[tempf$SYMBOL == tempf_sym[k], ]$AS_TYPE[1]
    temp <- data.table::fread(list.files(store_dir, pattern=paste0('^',tempf_uni,'_',tempf$CANCER[k]), full.names=TRUE)[1])

    exons <- unique(temp$EXON_hg38)
    for(j in 1:length(exons)){
        tempxx <- temp[temp$EXON_hg38 == exons[j], ]
        print(unique(tempxx$CHR_START_POS_CODING_hg38))
        print(unique(tempxx$CHR_END_POS_CODING_hg38))

    }

    wh <- which(colnames(temp) == temp_as)
    temp_chr <- unique(temp$CHR_NUM_hg38)

    ## which AS type --
    temp_wh1 <- which((temp[[wh]] != '')&(!is.na(temp[[wh]])))#temp[which((temp[[wh]] != '')&(!is.na(temp[[wh]]))), ]
    tempex1 <- unique(temp[temp_wh1, ]$EXON_hg38)
    tempnt1_wh1 <- min(temp[temp_wh1, ]$NT1_hg38)
    tempnt3_wh1 <- max(temp[temp_wh1, ]$NT3_hg38)
    print(paste0(tempf_sym[k],'\n'))
    print('*********** AS positions ***********\n')
    print(paste0(tempex1,'\n'))
    print(paste0(tempnt1_wh1,'\n'))
    print(paste0(tempnt3_wh1,'\n'))
    tempnt1_wh1 <- min(temp[temp_wh1, ]$UNIPROT_SEQ_NUM)
    tempnt3_wh1 <- max(temp[temp_wh1, ]$UNIPROT_SEQ_NUM)
    print(paste0(tempnt1_wh1,'\n'))
    print(paste0(tempnt3_wh1,'\n'))
    print('*************************\n')
    ## which DBD --
    temp_wh2 <- which(temp[[14]] != '-')
    if(length(temp_wh2) != 0){
        tempex2 <- unique(temp[temp_wh2, ]$EXON_hg38)
        tempnt1_wh2 <- min(temp[temp_wh2, ]$NT1_hg38)
        tempnt3_wh2 <- max(temp[temp_wh2, ]$NT3_hg38)
        print('*********** DBD positions ***********\n')
        print(paste0(tempex2,'\n'))
        print(paste0(tempnt1_wh2,'\n'))
        print(paste0(tempnt3_wh2,'\n'))
            tempnt1_wh2 <- min(temp[temp_wh2, ]$UNIPROT_SEQ_NUM)
    tempnt3_wh2 <- max(temp[temp_wh2, ]$UNIPROT_SEQ_NUM)
    print(paste0(tempnt1_wh2,'\n'))
    print(paste0(tempnt3_wh2,'\n'))
        print('*************************\n')
    }
    ...
    ## which ED --
    temp_wh3 <- which(temp[[21]] != '-')
    if(length(temp_wh3) != 0){
        tempex3 <- unique(temp[temp_wh3, ]$EXON_hg38)
        tempnt1_wh3 <- min(temp[temp_wh3, ]$NT1_hg38)
        tempnt3_wh3 <- max(temp[temp_wh3, ]$NT3_hg38)
        print('*********** ED positions ***********\n')
        print(paste0(tempex3,'\n'))
        print(paste0(tempnt1_wh3,'\n'))
        print(paste0(tempnt3_wh3,'\n'))
                    tempnt1_wh3 <- min(temp[temp_wh3, ]$UNIPROT_SEQ_NUM)
    tempnt3_wh3 <- max(temp[temp_wh3, ]$UNIPROT_SEQ_NUM)
    print(paste0(tempnt1_wh3,'\n'))
    print(paste0(tempnt3_wh3,'\n'))
        print('*************************\n')
    }
